...
class D2Options(Toplevel):
    ...
    def __init__(self, parent):
        ...
        unitvector.config(height="2",
                            ...
                            borderwidth="4",
                            activebackground="#00ff00")
        unitvector.pack(pady="0")

        test = Button(self, text="testing", font=("Times", 20), command=lambda: self.switch_frame(Testing))
        test.config(height="2",
                            width="14",
                            bg="black",
                            fg="#97e6ca",
                            relief="groove",
...
class Testing(Toplevel):
    ...
    def __init__(self,  parent):
        ...
        l = Label(self, text = "2D Unit Vector") 
        l.config(font =("Courier", 80), bg="#9c1019", fg="white", borderwidth="4", relief="groove", padx="20", pady="4")
        l.pack(pady="30")
        
        canvas1 = Canvas(self, width = 400, height = 300, bg="black")
        canvas1.place(x=780, y=230)
        ...
        a.set_title ("Vector Grid", fontsize=16)
        a.set_ylabel("Y", fontsize=14)
        a.set_xlabel("X", fontsize=14)

        canvas = FigureCanvasTkAgg(fig, master=self)
        canvas.get_tk_widget().place(x=40, y=180)
        canvas.draw()
        ...
        Calculate = Button(self, text="Calculate", font=("Times", 15), command=lambda: [answertext.config(text = str(round(1/sqrt((float(entryx2.get())-float(entryx1.get()))**2 
                                                                                                                ...
                                                                                                                + ", " 
                                                                                                                + str(float(entryy2.get())-float(entryy1.get())) 
                                                                                                                + ")"), a.plot(x, (float(entryy2.get())-float(entryy1.get()))/(float(entryx2.get())-float(entryx1.get()))*x + float(entryy1.get()) - (float(entryy2.get())-float(entryy1.get()))/(float(entryx2.get())-float(entryx1.get()))*float(entryx1.get()), '-r')])
        Calculate.config(height="2",
                        width="14",
                        bg="#94b3ae",
                        fg="purple",
                        activebackground="#00ff00")
        Calculate.place(x=900, y=560)
